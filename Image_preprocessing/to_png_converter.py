...
def any_to_png_converter(train_folder, val_folder):
    ...
    for filename in os.listdir(val_folder):
        if not filename.endswith(".png"):
            print("converting " + filename + " to png")
            img = Image.open(os.path.join(val_folder, filename))
            new_filename = os.path.splitext(filename)[0] + ".png"
            img.save(os.path.join(val_folder, new_filename))
            os.remove(os.path.join(val_folder, filename))
